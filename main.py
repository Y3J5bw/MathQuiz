...
class App(tk.Tk):
    ...
    # Set to static as tho it does not use any variables from the class
    @staticmethod
    def proceed():
        name = type(current_screen[0]).__name__

        event_dict = {'IntroScreen': intro_screen.login, 'Signup': register_screen.register,
                      'MainMenu': lambda: screen_change(current_screen, select_menu),
                      'SelectMenu': select_menu.proceed, 'Question': question.frame_check,
                      'ResultsScreen': lambda: screen_change(current_screen, main_menu)}
...
class IntroScreen(tk.Frame):
    ...
    def __init__(self, master):
        ...
        self.entry_frame = tk.Frame(self, background='#00a876')
        self.entry_frame.columnconfigure(0, weight=1)
        self.login_text = ttk.Label(self.entry_frame, text='Login or Create an Account',
                                    style='login.TLabel', font=('Helvetica', 30))
        self.username_text = ttk.Label(self.entry_frame, text='Username:', style='login.TLabel')
        self.username_entry = tk.Entry(self.entry_frame, font=('Helvetica', 15), foreground='#00a876', justify='center')
        self.password_text = ttk.Label(self.entry_frame, text='Password:', style='login.TLabel')
        self.password_entry = ttk.Entry(self.entry_frame, font=('Helvetica', 15), foreground='#00a876',
                                        justify='center', show='*')

        self.buttons_frame = tk.Frame(self.entry_frame, background='#00a876')
        self.register_button = ttk.Button(self.buttons_frame, text='Register', style='TButton',
                                          command=lambda: screen_change(self.master.current_screen,
                                                                        self.master.register_screen))
...
class SelectMenu(tk.Frame):
    ...
    def __init__(self, master):
        ...
        [self.rowconfigure(row, weight=weight) for row, weight in zip([0, 1, 2], [1, 2, 1])]
    ...
    def grid_frames(self):
        self.back_button.grid(row=0, column=0, sticky='nw', padx=20)
        self.select_text.grid(row=0, column=2, sticky='nsew', columnspan=3, pady=20)
...
def hash_password(password, salt=None):
    ...
    return hashlib.pbkdf2_hmac('sha256', str.encode(password), salt, 100000).hex(), salt
...
def screen_change(screen, new_screen):
    ...
    screen[0] = new_screen
    row = 0 if new_screen != intro_screen else 1
    screen[0].grid(row=row, column=0, sticky='nsew', pady=50)
