...
class LevenshteinDistance:
    """Class to calculat the Levenshtein distance and ratio of two strings.
    """
    def __init__(self, string1, string2):
        ...
        self.columns = len(string2) + 1

        self.array = [[0 for _ in range(self.columns)] for _ in range(1, 3)]

        for row in range(1, 2):
            self.array[row][0] = self.array[row - 1][0] + 1

        for column in range(1, self.columns):
            self.array[0][column] = self.array[0][column - 1] + 1

    def distance_calculation(self):
        for row in range(1, self.rows):
            for column in range(1, self.columns):
                subs_boolean = 0 if self.string1[row - 1] == self.string2[column - 1] else 1
                self.array[1][column] = min(self.array[0][column] + 1,
                                            self.array[1][column - 1] + 1,
                                            self.array[0][column - 1] + subs_boolean)
...
if __name__ == '__main__':
    a = [[], [], []]
    for i in range(3):
        a[0].append(timeit.timeit(test, number=100000))
        a[1].append(timeit.timeit(test2, number=100000))
        a[2].append(timeit.timeit(test3, number=100000))
    ...
    test = LevenshteinDistance("Ice Cream", "ice cream.")
    print(test.distance_calculation())
